...
function spritemanager:playFrame(tag, framename)
    ...
    if not spr.frames then
        if d then print("Tried to play frame " .. framename .. " but the sprite " .. spr.tag .. " has no frames") end
        return
    end

    local frame = spr.frames[framename]
    if not frame then
        if d then print("Frame \"" .. framename .. "\" not found in \"" .. spr.tag .. "\"") end
        return
    end

    if not frame.image then
        if d then print("Frame \"" .. framename .. "\" in \"" .. spr.tag .. "\" does not have an image") end
        return
    end
    ...
    if not path then
        ...
        for _,ext in ipairs(extensions) do
            for _,testpath in ipairs(paths) do
                ...
                if exists then
                    local cacheimage = spritemanager.cache[trypath]
                    if not cacheimage then
                        spritemanager.cache[trypath] = love.graphics.newImage(trypath)
                        cacheimage = spritemanager.cache[trypath]
                    end
                    return cacheimage, trypath
                end
            end
        end
...
function spritemanager:getSpriteData(data, extra)
    extra = extra or {}
    return {
        tag = data.tag,
        path = data.path,
...
function spritemanager:makeLuaSprite(tag, path, x, y, extra)
    ...
    if type(path) ~= "string" then
        image, newpath = path, path
    else
        image, newpath = self:findImage(path)
    end
    ...
    local spritedata = self:getSpriteData(data, extra)

    --table.insert(spritemanager.sprites, spritedata)
    self.sprites[tag] = spritedata
    if Game.debug then
        print(self.sprites[tag].tag .. " " .. tostring(self.sprites[tag].path))
    end

    return self.sprites[tag].tag
end
...
function spritemanager:removeLuaSprite(tag)
    ...
    -- i still think that sprite doesn't exist
end

function spritemanager:update(dt)
    for tag,spr in pairs(self.sprites) do
        if spr.collidable then
            spr.collidable.x, spr.collidable.y = spr.x, spr.y
        end
        if spr.scalebg then
            spr.x = 0
            spr.y = 0
            spr.sx = Game:getWidth() / spr.image:getWidth()
            spr.sy = Game:getHeight() / spr.image:getHeight()
        end
...
function spritemanager:_draw(spr)
    local sx = spr.sx * spr.mirror
    if spr.quad then
        love.graphics.draw(spr.image,spr.quad,spr.x,spr.y,spr.r,sx,spr.sy,spr.ox,spr.oy,spr.kx,spr.ky)
    else
        love.graphics.draw(spr.image,spr.x,spr.y,spr.r,sx,spr.sy,spr.ox,spr.oy,spr.kx,spr.ky)
    end
end
...
-- Function in charge of drawing the sprites
function spritemanager:draw(tag, consider)
    if consider == nil then consider = false end
    if not self.sprites then return end
    if tag then
        self:drawSprite(tag)
        return -- How could i have forget this lmao
    end
    ...
    for _,value in ipairs(self.drawSprites) do
        ...
        local nodraw = consider and (sprite.noautodraw and true or false)
        if not nodraw then
...
-- First parameter must be the object tag
function spritemanager:setObjectScale(tag, width, height)
    local spr = self:tagToSprite(tag)
...
function spritemanager:setObjectMirrored(tag, mirror)
    local spr = self:tagToSprite(tag)
    ...
    spr.mirror = mirror or -1
end

function spritemanager:setObjectBGScaled(tag, scaleBG)
    local spr = self:tagToSprite(tag)
    if scaleBG == nil then scaleBG = true end
    spr.scalebg = scaleBG
    spr.locked = scaleBG
end
...
function spritemanager:clearSprites()
    if self.onClearSprites then
        self:onClearSprites(self.sprites)
    end
    self.sprites = {}
end
